...
function t(;rep=rep,pct=pct,nIteration=nIteration,outFreq=outFreq,seed=seed,
	...
	my_w1      = zeros(Float32,my_batch_size)  #neural network weight between middle and output layers
	...
	if my_rank == 0
	    ...
	    missingIndex_inside_trainInd = zeros(Int32,length(missingIndex)) #missingIndex for trainInds (before it is for all individuals)
		for i in 1:length(missingIndex)
			missingIndex_inside_trainInd[i]=findfirst(x-> x==missingIndex[i], trainIndex)  #find the position of a genotyped individual
		end
		yobs_corr_missingIndex = yobs_corr[missingIndex_inside_trainInd] #vector of length nMissing
		...
		yobs_corr_missingIndex = zeros(Float32,length(missingIndex))
	end

	#broadcast from rank0 to other ranks
	yobs_corr_missingIndex = MPI.bcast(yobs_corr_missingIndex,0,comm)
	...
	for iter in 1:nIteration
	    for t in 1:my_batch_size #@Threads.threads
			...
			####################################################################
			# zn=un+e
      		# my_Z_array[t][missingIndex] = my_sol[t][ng_index_among_pedi] + randn(length(ng_index_among_pedi))*sqrt(my_σ2_e[t])#ng_index_among_pedi is pos of ng among 6k inds
      		dn = my_sol[t][ng_index_among_pedi] #dn=un
      		sampleZ!(yobs_corr_missingIndex, my_Z_array[t],
					 missingIndex, 
					 dn,
					 my_w1[t],
					 my_σ2_e[t], #changed: yobs_corr_missingIndex, my_Z_array[t], my_Z[:,t]
					 σ2_yobs;    #checked: yobs_corr_missingIndex, my_Z_array, my_Z is Float32
					 update_ycorr=true)
		end
		...
		if my_rank == 0
			...
			yobs_corr_missingIndex = yobs_corr[missingIndex_inside_trainInd] #ycorr is length of nTrain, find those with missing genotypes
			w1_all       = weights_NN[2:end]
			...
			#update VBuffer for Scatterv
			w1_all_vbuf = VBuffer(w1_all, batch_size_all)
		else
			w1_all_vbuf = VBuffer(nothing)
		end
		MPI.Barrier(comm) #new added
		
		#broadcast from rank0 to other ranks
		σ2_yobs, yobs_corr_missingIndex = MPI.bcast([σ2_yobs, yobs_corr_missingIndex], 0, comm) #checked!
		
		#send w1 from rank0 to all ranks
		my_w1 = MPI.Scatterv!(w1_all_vbuf, zeros(Float32, my_batch_size), 0, comm)
